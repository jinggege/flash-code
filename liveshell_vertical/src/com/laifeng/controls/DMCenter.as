package com.laifeng.controls
{
	...
	import lf.media.core.data.LiveCoreDataPublic;
	...
	public class DMCenter extends EventDispatcher implements IDataModule
	{
		...
		public function play(playOption:PlayOption):void{
			...
			_liveCoreDataP = new LiveCoreDataPublic();
			...
			_liveCoreDataP.addEventListener(StatusConfig.STATUS_ROOM,getRoomStatusHandler);
		...
		private function requestCompleteHandler(event:LfEvent):void{
			...
			switch(int(statusData.data)){
				case StatusConfig.STATUS_STEP_1:
					reportS["tryCount"] = _liveCoreDataP.requestTryCount;
					reportS["ct"]              = _liveCoreDataP.requestUseTime;
					_reportM.reportGetplaylistSucceed(reportS);
					break;
				case StatusConfig.STATUS_STEP_2:
		...
		/**频道状态*/
		private function getRoomStatusHandler(event:LfEvent):void{
			var statusData:StatusData = event.data as StatusData;
			switch(statusData.data){
				
				case StatusConfig.STATUS_ROOM_END:
					LiveConfig.liveStatus = 0;
					Notification.get.notify(new MEvent(NoticeKey.OPEN_UI,UIKey.UI_PLUGS));
					break;
				case StatusConfig.STATUS_ROOM_LIVING:
		...
		public function get getStreamType():String{
			return _liveCoreDataP.getStreamType;
		}
		...
		public function isShowQualitySelect():Boolean{
			return _liveCoreDataP.getTransitlistLength() >= 1;
		}
		
		
		public function getLiveCoreData():LiveCoreDataPublic{
		...
		private function clearLiveCoreData():void{
			if(_liveCoreDataP != null){
				...
				_liveCoreDataP.removeEventListener(StatusConfig.STATUS_ROOM,getRoomStatusHandler);
			}
			_liveCoreDataP = null;
		}
		...
		private var _liveCoreDataP     :LiveCoreDataPublic;
