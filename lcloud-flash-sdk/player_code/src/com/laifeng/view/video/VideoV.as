package com.laifeng.view.video
{
    ...
    import lf.media.core.video.LfP2pVideo;
    import lf.media.core.video.MediaType;
    ...
    public class VideoV extends Sprite implements IUI{
		...
		private function creatVideoByType():void{
			...
			LiveConfig.get.streamLogData.useP2P = _dmCenter.getLiveCoreData().isP2p;
			
				switch(_dmCenter.getStreamType){
					case StatusConfig.STAEAM_TYPE_HTTP:
							_cVideo = new LfNomalVideo(callbackByVideo);
							LiveConfig.get.streamLogData.cdn = int(_dmCenter.getLiveCoreData().cdn);
							_cVideo.creat(null);
						break;
					case StatusConfig.STAEAM_TYPE_P2P:
							_cVideo = new LfP2pVideo(callbackByVideo);
							LiveConfig.get.streamLogData.cdn = 3;
							
							var p2pConfig:Object = {};
							p2pConfig["channel_id"] = _dmCenter.getLiveCoreData().streamId;
							p2pConfig["area_code"]   =  _dmCenter.getLiveCoreData().area_code;
							p2pConfig["dma_code"]   =  _dmCenter.getLiveCoreData().dma_code;
							p2pConfig["session_id"]   = _dmCenter.getLiveCoreData().sessionId;
							p2pConfig["token"]           =  _dmCenter.getLiveCoreData().token;
							p2pConfig["addrDispatcherUrl"] =  _dmCenter.getLiveCoreData().dispatcherUrl;
							p2pConfig["bps"]              =  _dmCenter.getLiveCoreData().bps;
							p2pConfig["debug"]         = false;
							_cVideo.creat(p2pConfig);
						break;
		...
		private function callbackByVideo(data:CallbackData):void{
			...
			switch(data.callbackType){
				...
				case "NetStream.Seek.InvalidTime":
					...
					if(_lastSeek != frame){
						trace("=========skip frame=", frame - _lastSeek);
						_cVideo.netStream.seek(frame);
					}
					
					_lastSeek = frame;
					
					trace("=ns time=",_cVideo.netStream.time);
					trace(frame);
			...
			if(_cVideo.mediaType == MediaType.VIDEO_P2P){
				...
				//if(_cVideo.netStream.bufferLength >=  this._buffMax){
					if(_cVideo.netStream.bufferLength >=  1.5){
						//todo 待确认后再打开
						//_cVideo.netStream.bufferTime = 0.1;
						
						if(_cVideo.netStream.decodedFrames>5){
							if(!_isPause){
		...
		private function checkRoomStatus(roomStatus:int):void{
			switch(roomStatus){
			    case -1:
				case  1:
					this.close();
