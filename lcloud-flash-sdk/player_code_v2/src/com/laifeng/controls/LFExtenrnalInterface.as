package com.laifeng.controls
{
	...
	public class LFExtenrnalInterface
	{
		...
		public function start():void{
			...
			ExternalInterface.addCallback("startLive",     						this.startLive);
		...
		private function startLive(option:Object):int{
			...
			Notification.get.notify(
				new MEvent(NoticeKey.N_LIVE_START_BY_JS,option)
			);
			return 1;
		}
		...
		public function stopLive(type:int = 0):void{
			...
			Notification.get.notify(
				new MEvent(NoticeKey.N_LIVE_STOP_BY_JS,null)
			);
			
			var interfaceData:Object = {};
			interfaceData.interfaceType = InterfaceKey.ITYPE_PLAY_STOP;
			interfaceData.param = {};
		...
		public function setBarrageChatStatus(status:int):void{
			Notification.get.notify(
				new MEvent(NoticeKey.N_SET_BARRAGE,
		...
		public function switchRoom():void{
			ExternalInterface.call(InterfaceKey.IKEY_SWITCH_ROOM);
		...
		/**
		 * @全局调用JS
		 * @interfaceData： {interfaceType:xxxxx,param:{}}
		 */
		public function globelCallJs(interfaceData:Object):void{
			try{
				ExternalInterface.call(
					InterfaceKey.IKEY_PLAYER_CALL_JS,
					interfaceData);
			}catch(e:Error){};
